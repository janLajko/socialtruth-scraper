#!/usr/bin/env python3
"""
Fetch the newest Truth Social post by @realDonaldTrump, send it to a Lark
incoming webhook, and avoid re-sending posts already delivered.
"""
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        "--webhook",
        default=None,
        help="Lark (Feishu) incoming webhook URL. If omitted, the latest post is printed.",
    )
    parser.add_argument(
        "--state-file",
    ...
    last_sent_id = load_last_sent(state_path) if args.webhook else None

    if args.webhook and last_sent_id == post["id"]:
        print("Latest post already sent to Lark; nothing to do.")
        return 0
    if not args.webhook:
        json.dump(post, sys.stdout, ensure_ascii=False, indent=2)
        sys.stdout.write("\n")
        return 0
